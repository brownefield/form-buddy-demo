    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-primary { background: var(--teal); color: white; padding: 16px 36px; border-radius: 10px; font-weight: 600; font-size: 1.05rem; text-decoration: none; transition: all 0.2s; box-shadow: 0 4px 24px rgba(13,115,119,0.4); }
        .btn-primary:hover { background: #0a9396; transform: translateY(-2px); }
        .btn-secondary { background: rgba(255,255,255,0.1); color: white; padding: 16px 36px; border-radius: 10px; font-weight: 600; font-size: 1.05rem; text-decoration: none; border: 1px solid rgba(255,255,255,0.2); transition: all 0.2s; }
        .stats-bar { background: var(--off-white); border-bottom: 1px solid var(--light-border); padding: 32px 40px; display: flex; justify-content: center; gap: 80px; flex-wrap: wrap; }
        .stat { text-align: center; }
        .stat-num { font-family: 'Playfair Display', serif; font-size: 2.2rem; font-weight: 700; color: var(--navy); }
        ...
        .section-title { font-size: clamp(2rem, 4vw, 3rem); font-weight: 700; color: var(--navy); line-height: 1.15; margin-bottom: 20px; }
        ...
        .how-card { padding: 40px 32px; border-radius: 16px; background: var(--off-white); border: 1px solid var(--light-border); position: relative; transition: transform 0.2s, box-shadow 0.2s; }
        .how-card:hover { transform: translateY(-4px); box-shadow: 0 16px 48px rgba(26,46,74,0.1); }
        .how-num { font-family: 'Playfair Display', serif; font-size: 4rem; font-weight: 900; color: var(--teal-light); line-height: 1; position: absolute; top: 24px; right: 28px; }
        ...
        .vert-card { padding: 32px; border-radius: 14px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); transition: all 0.2s; }
        ...
        .vert-tag { display: inline-block; margin-top: 16px; background: rgba(13,115,119,0.3); color: #4ecdc4; padding: 4px 12px; border-radius: 100px; font-size: 0.78rem; font-weight: 600; }
        ...
        .price-card { padding: 40px 32px; border-radius: 16px; border: 2px solid var(--light-border); background: white; position: relative; transition: all 0.2s; }
        .price-card:hover { border-color: var(--teal); box-shadow: 0 16px 48px rgba(13,115,119,0.12); transform: translateY(-4px); }
        .price-card.featured { border-color: var(--teal); background: var(--navy); }
        .price-badge { position: absolute; top: -14px; left: 50%; transform: translateX(-50%); background: var(--teal); color: white; padding: 4px 16px; border-radius: 100px; font-size: 0.78rem; font-weight: 700; white-space: nowrap; }
        ...
        .price-features li { font-size: 0.92rem; color: var(--gray); display: flex; gap: 10px; }
        ...
        .price-btn { width: 100%; padding: 14px; border-radius: 10px; font-weight: 600; font-size: 0.95rem; text-align: center; text-decoration: none; display: block; transition: all 0.2s; background: var(--teal-light); color: var(--teal); }
        .price-btn:hover { background: var(--teal); color: white; }
        .price-card.featured .price-btn { background: var(--teal); color: white; }
        footer { background: var(--navy); color: rgba(255,255,255,0.5); padding: 60px 40px 40px; text-align: center; }
        ...
        @media (max-width: 768px) { nav { padding: 0 20px; } .nav-links { display: none; } .stats-bar { gap: 40px; padding: 24px 20px; } section { padding: 72px 20px; } .hero { padding: 100px 20px 60px; } }
      `}</style>
      ...
      <section id="how">
        <div className="container">
          ...
          <h2 className="section-title">From intake to outcome in four steps</h2>
          <p className="section-sub">FormBuddy handles the paperwork so you can focus on the work.</p>
      ...
      <section className="verticals" id="verticals">
        <div className="container">
          ...
          <h2 className="section-title">Built for the businesses that build Hawaii</h2>
          <p className="section-sub">Every vertical gets a purpose-built intake system, automation flow, and dashboard.</p>
          <div className="vert-grid">
            {[
              {icon:'🏗️',title:'Construction & Trades',desc:'Job intake, site inspection reports, subcontractor onboarding, and permit documentation.'},
              {icon:'🏥',title:'Healthcare & Billing',desc:'Physician onboarding, patient intake, prior auth workflows, and compliance documentation.'},
              {icon:'🌺',title:'Tourism & Hospitality',desc:'Guest intake, booking requests, incident reports, and vendor onboarding flows.'},
              {icon:'⛪',title:'Churches & Faith Orgs',desc:'Member intake, volunteer onboarding, ministry requests, and event registration.'},
              {icon:'🤝',title:'Nonprofits & Community',desc:'Client intake, service requests, grant documentation, and case management.'},
            ].map(({icon,title,desc}) => (
              <div className="vert-card" key={title}>
                <div className="vert-icon">{icon}</div>
                <h3>{title}</h3><p>{desc}</p>
                <span className="vert-tag">Available Now</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="pricing" style={{background:'var(--off-white)'}}>
        <div className="container" style={{textAlign:'center'}}>
          ...
          <div className="pricing-grid">
            {[
              {tier:'Starter',name:'Intake',price:'$149',features:['1 intake form','Excel auto-logging','Email-ready summaries','Up to 100 submissions/mo','Email support'],featured:false},
              {tier:'Most Popular',name:'Professional',price:'$299',features:['Up to 5 forms','SharePoint integration','Dashboard + analytics','Approval routing','Up to 500 submissions/mo','Priority support'],featured:true},
              {tier:'Scale',name:'Enterprise',price:'$599',features:['Unlimited forms','Custom automation flows','Multi-user access','White-label option','Unlimited submissions','Dedicated onboarding'],featured:false},
            ].map(({tier,name,price,features,featured}) => (
              <div className={`price-card ${featured?'featured':''}`} key={name}>
                ...
                <h3>{name}</h3>
                <div style={{marginTop:16,marginBottom:4}}><span className="price-amount">{price}</span><span className="price-period">/month</span></div>
                <div className="price-divider"/>
                <ul className="price-features">{features.map(f=><li key={f}>{f}</li>)}</ul>
      ...
      <section id="contact" style={{background:'linear-gradient(135deg, var(--teal) 0%, #0a5c60 100%)',textAlign:'center'}}>
        <div className="container">
          <h2 style={{fontFamily:'Playfair Display, serif',fontSize:'clamp(2rem, 4vw, 3rem)',color:'white',marginBottom:20}}>Ready to run your business on FormBuddy?</h2>
          <p style={{color:'rgba(255,255,255,0.75)',fontSize:'1.1rem',maxWidth:520,margin:'0 auto 40px'}}>Join Hawaii service businesses already automating their intake, workflows, and client management.</p>
      ...
      <footer>
        ...
        <div className="footer-links">
          <a href="#how">How It Works</a><a href="#verticals">Verticals</a>
          <a href="#pricing">Pricing</a><a href="mailto:hello@formbuddy.co">Contact</a>
        </div>
